...
public class HoseReelVerifyActivity extends AppCompatActivity {

    private static final String TAG = HoseReelVerifyActivity.class.getSimpleName();
    Context mContext;
    Toolbar toolbar;
    ...
    String modelNo = "", productId = "", location = "", spare_part_label = "", remarks = "", spare_part_item_label = "",
            client_name = "", hose_reel = "", shut_off_nozzel = "", ball_valve = "", jubli_clip = "", conecting_ruber_hose = "",
            observation = "", action = "";
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        location = getIntent().getStringExtra(Constant.HoseReelInfo.location);
        spare_part_label = getIntent().getStringExtra(Constant.HoseReelInfo.spare_part_label);

        hose_reel = getIntent().getStringExtra(Constant.HoseReelInfo.hose_reel);
        shut_off_nozzel = getIntent().getStringExtra(Constant.HoseReelInfo.shut_off_nozzel);
        ball_valve = getIntent().getStringExtra(Constant.HoseReelInfo.ball_valve);
        jubli_clip = getIntent().getStringExtra(Constant.HoseReelInfo.jubli_clip);
        conecting_ruber_hose = getIntent().getStringExtra(Constant.HoseReelInfo.conecting_ruber_hose);
        observation = getIntent().getStringExtra(Constant.HoseReelInfo.observation);
        action = getIntent().getStringExtra(Constant.HoseReelInfo.action);
    ...
    private void insertNewProduct() {
        ...
        Call<JsonObject> call = apiInterface.createHoseReel(Utility.getSharedPreferences(mContext, Constant.userId),
                modelNo, productId, location, spare_part_label, remarks, spare_part_item_label, client_name, observation, action,
                hose_reel, shut_off_nozzel, ball_valve, jubli_clip, conecting_ruber_hose);
